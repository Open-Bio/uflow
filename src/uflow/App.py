...
from uflow.Wizards.PackageWizard import PackageWizard
...
class uflow(QMainWindow):
    ...
    def __init__(self, parent=None):
        super(uflow, self).__init__(parent=parent)
    ...
    def loadFromData(self, data, clearHistory=False):
        ...
        missedPackages = (
            set()
        )  # TODO: nothing fills this, can never report missing package
    ...
    def save(self, save_as=False):
        if save_as:
            name_filter = "Graph files (*.pygraph)"
            default_name = (
                "untitled" if self.currentFileName is None else self.currentFileName
            )
            savepath = QFileDialog.getSaveFileName(
                filter=name_filter, directory=default_name
            )
            if type(savepath) in [tuple, list]:
                pth = savepath[0]
            else:
            ...
            if self.currentFileName is None:
                name_filter = "Graph files (*.pygraph)"
                savepath = QFileDialog.getSaveFileName(
                    filter=name_filter, directory="untitled"
                )
                if type(savepath) in [tuple, list]:
                    pth = savepath[0]
                else:
        ...
        if not self.currentFileName == "":
            ...
            tempFileName = (
                self.currentFileName.replace(".pygraph", "")
                + f"-tempfile-{current_time}.pygraph"
            )
            ...
            try:
                ...
                raise RuntimeError(
                    f'JSON serialization failed.\nCould not save file "{self.currentFileName}"'
                ) from e
            ...
            for ToolClass in registeredToolSet:
                ...
                if issubclass(ToolClass, DockTool):
                    ...
                    showToolAction.triggered.connect(
                        lambda x=0,
                        pkgName=packageName,
                        toolName=ToolClass.name(): instance.invokeDockToolByName(

...
from uflow.UI.Canvas.CanvasBase import CanvasBase

import uflow.UI.resources
...
def winTitle():
    return "uflow v{0}".format(currentVersion().__str__())


class uflowChild(QMdiSubWindow):
    sequenceNumber = 1
    newFileExecuted = QtCore.Signal(bool)
    fileBeenLoaded = QtCore.Signal()

    def __init__(self, parent):
        super(uflowChild, self).__init__(parent)
    ...
    def getTempDirectory(self):
        ...
        if self.currentTempDir == "":
            ...
            tempDirPath = ConfigManager().getPrefsValue(
                "PREFS", "GeneralPreferences/TempFilesDir"
            )
            if tempDirPath[-1:] in ("/", "\\"):
        ...
        for packageName, registeredToolSet in GET_TOOLS().items():
            for ToolClass in registeredToolSet:
                ...
                if issubclass(ToolClass, FormTool):
                    ...
                    pluginsMenuAction = [m for m in menus if m.title() == "Tools"][
                        0
                    ].menuAction()
                ...
                if issubclass(ToolClass, DockTool):
                    ...
                    showToolAction.triggered.connect(
                        lambda pkgName=packageName,
                        toolName=ToolClass.name(): self.invokeDockToolByName(
                            pkgName, toolName
                        )
                    )
                    settings.beginGroup("DockTools")
    ...
    def newFile(self):
        self.isUntitled = True
        self.curFile = f"document{uflowChild.sequenceNumber}.pygraph"
        self._currentFileName = f"document{uflowChild.sequenceNumber}.pygraph"
        uflowChild.sequenceNumber += 1
...
class MDIMain(QMainWindow):
    ...
    def populateMenu(self):
            ...
            for exporterName, exporterClass in exporters.items():
                ...
                if exporterClass.createExporterMenu():
                    ...
                    exportAction.triggered.connect(
                        ...
                        app=self,
                        exporter=exporterClass: exporter.doExport(app)
                    )
                if exporterClass.createImporterMenu():
                    ...
                    importAction.triggered.connect(
                        lambda checked=False,
                        app=self,
                        exporter=exporterClass: exporter.doImport(app)
                    )
        ...
        helpMenu.addAction("Homepage").triggered.connect(
            lambda _=False,
            url="https://pedrocabrera.github.io/uflow/": QtGui.QDesktopServices.openUrl(
                url
            )
        )
        helpMenu.addAction("Docs").triggered.connect(
            lambda _=False,
            url="https://uflow.readthedocs.io/en/latest/": QtGui.QDesktopServices.openUrl(
                url
            )
        )
    ...
    def refreshToolBar(self, instance):
        if (
            instance is None
        ):  # The Package is selected but a file has not be loaded/started
            package = "Root"
    ...
    def updateToolBar(self, tBar, tDict):
        if tDict["Action"] == "Add Action":
            toolAction = self.actionRegisterDict[tDict["Package"]].getAction(
                tDict["PackageGroup"], tDict["Command"]
            )
            try:

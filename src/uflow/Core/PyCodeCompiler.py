from uflow.Core.Interfaces import ICodeCompiler
...
class Py3FunctionCompiler(ICodeCompiler):
    ...
    def compile(self, code):
        ...
        if len(lines) == 0:
            foo += "\n\tpass"
        codeObject = compile(foo, "uflowCodeCompiler", "exec")
        mem = {}
        exec(codeObject, mem)
...
class Py3CodeCompiler(ICodeCompiler):
    ...
    def compile(self, code, moduleName="uflowCodeCompiler", scope=None):

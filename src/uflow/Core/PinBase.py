...
from uflow.Core.EvaluationEngine import EvaluationEngine
from uflow.Core.Interfaces import IPin
...
class PinBase(IPin):
    ...
    **Base class for pins**
    ...
    :param _packageName: This holds the package where the sub classed pin is registered.
                         It is not intended to be set by developer, uflow automatically fills this property at
                         registration point
    ...
    Signals:
        ...
        * **dictChanged** : Fired when current structure changes to :py:const:`uflow.Core.Common.StructureType.Dict`, sends Dict key DataType
    ...
    :ivar reconnectionPolicy: What to do if connect with busy pin. Used when :attr:`~uflow.Core.Common.PinOptions.AllowMultipleConnections` flag is disabled
    ...
    def disableOptions(self, *options):
        """Same as :meth:`~uflow.Core.PinBase.PinBase.enableOptions` but inverse"""
        for option in options:
            self._flags = self._flags & ~option
        self._origFlags = self._flags
    ...
    def optionEnabled(self, option):
        ...
        :type option: :class:`~uflow.Core.Common.PinOptions`
        :rtype: bool
        """
    ...
    def linkedTo(self):
        ...
        if self.direction == PinDirection.Output:
            for i in getConnectedPins(self):
                connection = {
                    "lhsNodeName": self.owningNode().getName(),
                    "outPinId": self.pinIndex,
                    "rhsNodeName": i.owningNode().getName(),
                    "inPinId": i.pinIndex,
                    "lhsNodeUid": str(self.owningNode().uid),
                    "rhsNodeUid": str(i.owningNode().uid),
                }
        ...
        if self.direction == PinDirection.Input:
            for i in getConnectedPins(self):
                connection = {
                    "lhsNodeName": i.owningNode().getName(),
                    "outPinId": i.pinIndex,
                    "rhsNodeName": self.owningNode().getName(),
                    "inPinId": self.pinIndex,
                    "lhsNodeUid": str(i.owningNode().uid),
                    "rhsNodeUid": str(self.owningNode().uid),
                }
                result.append(connection)
        return result
    ...
    def setName(self, name, force=False):
        ...
        :type name: str
        :param force: If True - name will be changed even if option :attr:`~uflow.Core.Common.PinOptions.RenamingEnabled` is turned off
        :type force: bool
    ...
    def getData(self):
        ...
        .. seealso:: :class:`~uflow.Core.EvaluationEngine.DefaultEvaluationEngine_Impl`
        """
    ...
    def aboutToConnect(self, other):
        ...
        :type other: :class:`~uflow.Core.PinBase.PinBase`
        """
    ...
    def changeStructure(self, newStruct, init=False):
        ...
        :type newStruct: :class:`~uflow.Core.Common.StructureType`
    ...
    def canChangeTypeOnConnection(
        self, checked=None, can=True, extraPins=None, selfCheck=True
    ):
    ...
    def getDictElementNode(self, checked=None, node=None):
        ...
        :rtype: :class:`~uflow.Core.NodeBase.NodeBase` or None
        """
        if checked is None:
            checked = []

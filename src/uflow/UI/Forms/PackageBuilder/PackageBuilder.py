...
RESOURCES_DIR = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "UI/resources/"
)
...
class TableComboModel(QComboBox):
    ...
    def data(self, index, role=QtCore.Qt.DisplayRole):
        ...
        if not index.isValid() or (
            role != QtGui.Qt.DisplayRole and role != QtGui.Qt.EditRole
        ):
        ...
        if 1 == 2:
            if newName != "":
                ...
                for row in range(self.dataout["dataModel"].rowCount()):
                    cmbTableModel.setItem(
                        row + 1,
                        1,
                        QStandardItem(self.dataout["dataModel"].index(row, 1).data()),
                    )
...
class PackageBuilder(QMdiSubWindow):
    def __init__(self, parent=None):
        ...
        self.ui.cmdCommandAddSmallIcon.clicked.connect(
            self.on_cmdCommandAddSmallIcon_clicked
        )
        self.ui.cmdCommandAddMediumIcon.clicked.connect(
            self.on_cmdCommandAddMediumIcon_clicked
        )
        self.ui.cmdCommandAddLargeIcon.clicked.connect(
            self.on_cmdCommandAddLargeIcon_clicked
        )
    ...
    def getMenuLayout(self):
        ...
        packageToolList = []
        packageToolList.append(
            {
                "Action": "Add Action",
                "Package": "PackageBuilder",
                "PackageGroup": "ProgramBase",
                "Instance": self,
        ...
        ribbonItems.append(
            {
                ...
                "Section": "Tools",
                "Widget": "Small Button",
                "SmallIcon": RESOURCES_DIR + "new_file_icon.png",
                "LargeIcon": RESOURCES_DIR + "new_file_icon.png",
                "Action": "Add Action",
                "Package": "ProgramBase",
                "PackageGroup": "ProgramBase",
                "Instance": self,
    ...
    def onChangeFilterValue(self, text):
        ...
        search = QtCore.QRegularExpression(
            str(text), QRegularExpression.CaseInsensitiveOption
        )
    ...
    def on_tvPackageItems_clicked(self, it, col):
        ...
        selectedpackage = (
            self.ui.lstPackages.model()
            .index(self.ui.lstPackages.currentIndex().row(), 0)
            .data()
        )
            ...
            try:
                ...
                if "CATEGORY" in self.functiondict[deft]["MetaData"]:
                    self.ui.txtMetaCategory.setText(
                        self.functiondict[deft]["MetaData"]["CATEGORY"]
                    )
                if "KEYWORDS" in self.functiondict[deft]["MetaData"]:
                    self.ui.txtMetaKeywords.setText(
                        self.functiondict[deft]["MetaData"]["KEYWORDS"]
                    )
                if "CacheEnabled" in self.functiondict[deft]["MetaData"]:
                    self.ui.chkMetaCacheEnabled.setChecked(
                        self.functiondict[deft]["MetaData"]["CacheEnabled"]
                    )
    ...
    def loadFunctionProperties(self, filefullpath, functionname):
        ...
        try:
            ...
            for index, lineitem in enumerate(f):
                ...
                if lineitem.find("@staticmethod") != -1 or index == filesize - 1:
                    ...
                    if defname == functionname:
                        if precode.find("@staticmethod") != -1:
                            ...
                            NestDict["Implement"] = implement2.replace(
                                "@staticmethod", ""
                            )
                ...
                if (
                    lineitem.find("def ") != -1
                    and lineitem.find(" " + functionname) != -1
                ):
                    defnamestart = 7
                ...
                if (
                    lineitem.find("@IMPLEMENT_NODE") != -1
                ) or readingimplementdata == 1:
            ...
            if codedata[defitem].find("getIcon") != -1:
                for row in range(defitem, endCodeBlock):
                    ...
                    if codedata[row].find("return") != -1:
                        ...
                        self.ui.cmdCommandAddSmallIcon.setIcon(
                            QtGui.QIcon(os.path.join(RESOURCES_DIR, filename))
                        )
    ...
    def parseFunctionFile(self, defname):
        ...
        implementdata2.append(
            implementdata[curlbracketstart - 6 : curlbracketend - 1] + "})"
        )
        ...
        if data != "None":
            ...
            pindata["DataType"] = (
                listdata[0][1:].strip().replace('"', "").replace("'", "")
            )
        ...
        for variable, data in defs.items():
            ...
            if data.find("REF") == -1:
                ...
                pindata["DataType"] = (
                    listdata[0].strip().replace('"', "").replace("'", "")
                )
        ...
        if "PinDefs" in self.functiondict[defname]:
            ...
            if "Inputs" in self.functiondict[defname]["PinDefs"]:
                for pindata in self.functiondict[defname]["PinDefs"]["Inputs"]:
                    row = (
                        int(
                            self.functiondict[defname]["PinDefs"]["Inputs"][pindata][
                                "Order"
                            ]
                        )
                        - 1
                    ...
                    if (
                        "DataType"
                        in self.functiondict[defname]["PinDefs"]["Inputs"][pindata]
                    ):
                        inputpinlistmodel.setItem(
                            row,
                            1,
                            QtGui.QStandardItem(
                                self.functiondict[defname]["PinDefs"]["Inputs"][
                                    pindata
                                ]["DataType"]
                            ),
                        )
                        DataTypeValue = self.functiondict[defname]["PinDefs"]["Inputs"][
                            pindata
                        ]["DataType"]
                ...
                inputpinlistmodel.setHeaderData(
                    0, QtCore.Qt.Horizontal, "Name", role=QtCore.Qt.DisplayRole
                )
                inputpinlistmodel.setHeaderData(
                    1, QtCore.Qt.Horizontal, "Data Type", role=QtCore.Qt.DisplayRole
                )
                inputpinlistmodel.setHeaderData(
                    2, QtCore.Qt.Horizontal, "Default Value", role=QtCore.Qt.DisplayRole
                )

            outputPinList = []
            if "Outputs" in self.functiondict[defname]["PinDefs"]:
                outputpinlistmodel = QtGui.QStandardItemModel(0, 2)
                for rowcount2, pindata in enumerate(
                    self.functiondict[defname]["PinDefs"]["Outputs"]
                ):
                    row = (
                        int(
                            self.functiondict[defname]["PinDefs"]["Outputs"][pindata][
                                "Order"
                            ]
                        )
                        - 1
                    ...
                    if (
                        "DataType"
                        in self.functiondict[defname]["PinDefs"]["Outputs"][pindata]
                    ):
                        outputpinlistmodel.setItem(
                            row,
                            1,
                            QtGui.QStandardItem(
                                self.functiondict[defname]["PinDefs"]["Outputs"][
                        ...
                        DataTypeValue = self.functiondict[defname]["PinDefs"][
                            "Outputs"
                        ][pindata]["DataType"]
                ...
                outputpinlistmodel.setHeaderData(
                    0, QtCore.Qt.Horizontal, "Name", role=QtCore.Qt.DisplayRole
                )
                outputpinlistmodel.setHeaderData(
                    1, QtCore.Qt.Horizontal, "Data Type", role=QtCore.Qt.DisplayRole
            ...
            if "Inputs" in self.functiondict[defname]["PinDefs"]:
                for row, data in enumerate(
                    ...
                    self.ui.tblFInputPins.openPersistentEditor(
                        pindatatypemodel.index(row, 1)
                    )
                    c = TableComboModel(
                        self, dataModel=pindatatypemodel, id=row, row=row, column=1
                    )
                    c.setValue(
                        self.functiondict[defname]["PinDefs"]["Inputs"][data][
                            "DataType"
                        ],
                        1,
                    )
            ...
            if "Outputs" in self.functiondict[defname]["PinDefs"]:
                for row, data in enumerate(
                    ...
                    c.setValue(
                        self.functiondict[defname]["PinDefs"]["Outputs"][data][
                            "DataType"
                        ],
                        1,
                    )
    ...
    def on_chkPSSupportedDataTypes_stateChanged(self):
        ...
        self.pindefs[self.selectedPinDir][self.selectedPinName][
            "SUPPORTED_DATA_TYPES"
        ] = self.ui.txtPSSupportedDataTypes.text()
    ...
    def on_chkPSConstraint_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["CONSTRAINT"] = (
            self.ui.txtPSConstraint.text()
        )
    ...
    def on_chkPSStructConstraint_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["STRUCT_CONSTRAINT"] = (
            self.ui.txtPSStructConstraint.text()
        )
    ...
    def on_chkPSInputWidget_stateChanged(self):
        ...
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName][
            "INPUT_WIDGET_VARIANT"
        ] = self.ui.txtPSInputWidget.text()
    ...
    def on_chkPSDescription_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["DESCRIPTION"] = (
            self.ui.txtPSDescription.text()
        )
    ...
    def on_chkPSValueList_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["VALUE_LIST"] = (
            self.ui.txtPSValueList.text()
        )
    ...
    def on_chkPSValueRange_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["VALUE_RANGE"] = (
            self.ui.txtPSValueRange.text()
        )
    ...
    def on_chkPSDraggerSteps_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["DRAGGER_STEPS"] = (
            self.ui.txtPSDraggerSteps.text()
        )
    ...
    def on_chkArraySupported_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["ArraySupported"] = (
            self.ui.chkArraySupported.isChecked()
        )
    ...
    def on_chkDictionarySupported_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["DictSupported"] = (
            self.ui.chkDictionarySupported.isChecked()
        )
    ...
    def on_chkSupportOnlyArrays_stateChanged(self):
        ...
        self.pindefs[self.selectedPinDir][self.selectedPinName][
            "SupportsOnlyArrays"
        ] = self.ui.chkSupportOnlyArrays.isChecked()
    ...
    def on_chkAllowMultipleConnections_stateChanged(self):
        ...
        ] = self.ui.chkAllowMultipleConnections.isChecked()
    ...
    def on_chkChangeTypeOnConnection_stateChanged(self):
        ...
        ] = self.ui.chkChangeTypeOnConnection.isChecked()

    # @QtCore.pyqtSlot(int)
    def on_chkRenamingEnabled_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["RenamingEnabled"] = (
            self.ui.chkRenamingEnabled.isChecked()
        )
    ...
    def on_chkDynamic_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["Dynamic"] = (
            self.ui.chkDynamic.isChecked()
        )
    ...
    def on_chkAlwaysPushDirty_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["AlwaysPushDirty"] = (
            self.ui.chkAlwaysPushDirty.isChecked()
        )
    ...
    def on_chkStorable_stateChanged(self):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["Storable"] = (
            self.ui.chkStorable.isChecked()
        )
    ...
    def on_chkAllowAny_stateChanged(self, value):
        self.setFunctionDirty()
        self.pinDictCheck(self.selectedPinDir, self.selectedPinName)
        self.pindefs[self.selectedPinDir][self.selectedPinName]["AllowAny"] = (
            self.ui.chkAllowAny.isChecked()
        )
    ...
    def on_chkDictionaryElementSupported_stateChanged(self):
        ...
        ] = self.ui.chkDictionaryElementSupported.isChecked()
    ...
    def pinDictCheck(self, Direction, PinName):
        ...
        if (
            PinName not in self.pindefs["Inputs"]
            and PinName not in self.pindefs["Outputs"]
        ):
            ...
            for lineitem in f:
                if lineitem.find("def ") != -1:
                    if lineitem[8] != "_":
                        ...
                        classnameend = lineitem.find("(")
                        if (
                            self.workingFile.find(
                                lineitem[classnamestart + 1 : classnameend]
                            )
                            == -1
                        ):
    ...
    def on_cmdSaveCommand_clicked(self):
        ...
        with open(filefullpath, "w") as f:
            ...
            f.write("from uflow.UI.Tool.Tool import ShelfTool\n")
            f.write("from uflow.Core.Common import Direction\n")
            f.write("from qtpy import QtGui\n")
            f.write(
                "from uflow.Packages.%s.Tools import RESOURCES_DIR\n\n"
                % (selectedpackage)
            )
    ...
    def writeFunction(self):
        ...
        if "out" in self.pindefs["Outputs"]:
            ...
            if "SUPPORTED_DATA_TYPES" in data:
                pinspecifiers += (
                    ...
                    + data["SUPPORTED_DATA_TYPES"]
                    + ", "
                )
            if "CONSTRAINT" in data:
                pinspecifiers += (
                    "PinSpecifiers.CONSTRAINT: " + data["CONSTRAINT"] + ", "
                )
            if "STRUCT_CONSTRAINT" in data:
                pinspecifiers += (
                    "PinSpecifiers.STRUCT_CONSTRAINT: "
                    + data["STRUCT_CONSTRAINT"]
                    + ", "
                )
            if "ENABLED_OPTIONS" in data:
                pinspecifiers += (
                    "PinSpecifiers.ENABLED_OPTIONS: " + data["ENABLED_OPTIONS"] + ", "
                )
            if "DISABLED_OPTIONS" in data:
                pinspecifiers += (
                    "PinSpecifiers.DISABLED_OPTIONS: " + data["DISABLED_OPTIONS"] + ", "
                )
            if "INPUT_WIDGET_VARIANT" in data:
                pinspecifiers += (
                    "PinSpecifiers.INPUT_WIDGET_VARIANT: "
                    + data["INPUT_WIDGET_VARIANT"]
                    + ", "
                )
            if "DESCRIPTION" in data:
                pinspecifiers += (
                    "PinSpecifiers.DESCRIPTION: '" + data["DESCRIPTION"] + "', "
                )
            if "VALUE_LIST" in data:
                pinspecifiers += (
                    "PinSpecifiers.VALUE_LIST: " + data["VALUE_LIST"] + ", "
                )
            if "VALUE_RANGE" in data:
                pinspecifiers += (
                    "PinSpecifiers.VALUE_RANGE: " + data["VALUE_RANGE"] + ", "
                )
            if "DRAGGER_STEPS" in data:
                pinspecifiers += (
                    "PinSpecifiers.DRAGGER_STEPS: " + data["DRAGGER_STEPS"] + ", "
                )
            if pinspecifiers != "":
        ...
        if "CATEGORY" in self.functiondict[defname]["MetaData"]:
            mdata += (
                "NodeMeta.CATEGORY: "
                + self.functiondict[defname]["MetaData"]["CATEGORY"]
                + ", "
            )
        if "KEYWORDS" in self.functiondict[defname]["MetaData"]:
            mdata += (
                "NodeMeta.KEYWORDS: "
                + self.functiondict[defname]["MetaData"]["KEYWORDS"]
                + ", "
            )
        if "CACHE_ENABLED" in self.functiondict[defname]["MetaData"]:
            mdata += (
                "NodeMeta.CACHE_ENABLED: "
                + self.functiondict[defname]["MetaData"]["CACHE_ENABLED"]
                + ", "
            )
    ...
    def on_tblFInputPins_clicked(self, index):
        ...
        self.selectedPinName = (
            self.ui.tblFInputPins.model().index(index.row(), 0).data()
        )

        if self.selectedPinName in self.pindefs[self.selectedPinDir]:
            self.selectedPinData = self.pindefs[self.selectedPinDir][
                self.selectedPinName
            ]
    ...
    def on_tblFInputPins_doubleclicked(self, index):
        print(
            "Double Click", self.ui.tblFInputPins.model().index(index.row(), 0).data()
        )
        print("Row %d and Column %d was clicked" % (index.row(), index.column()))
        print(self.ui.tblFInputPins.model().data(index, QtCore.Qt.UserRole))
    ...
    def on_tblFOutputPins_clicked(self, index):
        ...
        self.ReferenceNumber_id = (
            self.ui.tblFOutputPins.model().index(index.row(), 0).data()
        )
    ...
    def loadPinData(self, data):
        ...
        if "ENABLED_OPTIONS" in data:
            ...
            for option in options:
                ...
                if (
                    "SupportsOnlyArrays" in option
                ):  #: Pin will only support other pins with array data structure
                    self.ui.chkSupportOnlyArrays.setChecked(True)
                if (
                    "AllowMultipleConnections" in option
                ):  #: This enables pin to allow more that one input connection. See :func:`~uflow.Core.Common.connectPins`
                ...
                if "=" in item:
                    ...
                    if pinOptionsStart != -1:
                        ...
                        if "headerColor" in phrase[0]:
                            self.selectedNodeData[defname]["HeaderColor"] = phrase[
                                1
                            ].strip()
                            self.ui.txtNodeHeaderColor.setText(
                                self.selectedNodeData[defname]["HeaderColor"]
                            )
    ...
    def loadNodePinTable(self):
        ...
        if "PinDefs" in self.selectedNodeData[defname]:
            ...
            if "Inputs" in self.selectedNodeData[defname]["PinDefs"]:
                for rowcount1, pindata in enumerate(
                    self.selectedNodeData[defname]["PinDefs"]["Inputs"]
                ):
                    row = (
                        int(
                    ...
                    if (
                        "DataType"
                        in self.selectedNodeData[defname]["PinDefs"]["Inputs"][pindata]
                    ):
                        inputpinlistmodel.setItem(
                            row,
                            1,
                        ...
                        DataTypeValue = self.selectedNodeData[defname]["PinDefs"][
                            "Inputs"
                        ][pindata]["DataType"]
                    inputPinList.append(DataTypeValue)
                    if (
            ...
            if "Outputs" in self.selectedNodeData[defname]["PinDefs"]:
                for rowcount2, pindata in enumerate(
                    ...
                    if (
                        "DataType"
                        in self.selectedNodeData[defname]["PinDefs"]["Outputs"][pindata]
                    ):
                        outputpinlistmodel.setItem(
                            row,
                            1,
            ...
            if "Inputs" in self.selectedNodeData[defname]["PinDefs"]:
                for row, data in enumerate(
                    ...
                    self.ui.tblNInputPins.openPersistentEditor(
                        pindatatypemodel.index(row, 1)
                    )
                    c = TableComboModel(
                        self, dataModel=pindatatypemodel, id=row, row=row, column=1
                    ...
                    c.setValue(
                        self.selectedNodeData[defname]["PinDefs"]["Inputs"][data][
                            "DataType"
                        ],
                        1,
                    )
            ...
            if "Outputs" in self.selectedNodeData[defname]["PinDefs"]:
                for row, data in enumerate(
                    ...
                    self.ui.tblNOutputPins.openPersistentEditor(
                        pindatatypemodel.index(row, 1)
                    )
                    c = TableComboModel(
                        self, dataModel=pindatatypemodel, id=row, row=row, column=1
                    ...
                    c.setValue(
                        self.selectedNodeData[defname]["PinDefs"]["Outputs"][data][
                            "DataType"
                        ],
                        1,
                    )
    ...
    def addCopyright(self, f):
        f.write(
            "## Copyright "
            + self.ui.txtCopyrightAuthor.text()
            + " "
            + self.ui.dteCopyrightYear.value()
        )
    ...
    def addIntro(self, f):
        ...
        f.write("from uflow.core.PackageBase import PackageBase\n")

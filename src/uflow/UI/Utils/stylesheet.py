...
class editableStyleSheet:
    def __init__(self, appInstance=None):
        ...
        try:
            themeName = ConfigManager().getPrefsValue(
                "PREFS", "ThemePreferences/Theme_Name"
            )
            if themeName:
    ...
    def loadFromData(self, data):
        if list(data.keys())[0] == "uflowStyleSheet":
            data = data["uflowStyleSheet"]

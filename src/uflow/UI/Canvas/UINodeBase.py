...
from uflow.UI.Canvas.UIPinBase import UIPinBase, getUIPinInstance, PinGroup
from uflow.UI.EditorHistory import EditorHistory
from uflow.UI.Canvas.UICommon import *
from uflow.UI.Widgets.InputWidgets import createInputWidget
from uflow.UI.Canvas.Painters import NodePainter
from uflow.UI.Widgets.PropertiesFramework import CollapsibleFormWidget
from uflow.UI.UIInterfaces import IPropertiesViewSupport
from uflow.UI.UIInterfaces import IUINode
...
class UINodeBase(QGraphicsWidget, IPropertiesViewSupport, IUINode):
    ...
    def __init__(
        ...
        # Computing time label
        self.computingTimeLabel = QGraphicsTextItem(self)
        self.computingTimeLabel.setDefaultTextColor(QtGui.QColor(255, 255, 0, 200))
        timeFont = QtGui.QFont("Consolas", 5)
        self.computingTimeLabel.setFont(timeFont)
        self.computingTimeLabel.setPlainText("")
        self.computingTimeLabel.setZValue(100)
        self.showComputingTime = True
    ...
    def finalizeRename(self, accepted=False):
        """Called by :class:`~uflow.UI.Canvas.UINodeBase.NodeName`
    ...
    def onComputed(self, *args, **kwargs):
        ...
        # Update computing time label
        if self._rawNode._computingTime is not None:
            time_str = str(self._rawNode._computingTime)
            # Format to show only milliseconds if very fast
            if self._rawNode._computingTime.total_seconds() < 1:
                ms = self._rawNode._computingTime.total_seconds() * 1000
                time_str = f"{ms:.2f}ms"
            else:
                time_str = f"{self._rawNode._computingTime.total_seconds():.3f}s"
            self.computingTimeLabel.setPlainText(time_str)
            self.updateComputingTimeLabelPosition()
        else:
            self.computingTimeLabel.setPlainText("")
            self.updateComputingTimeLabelPosition()
        self.update()
    ...
    def updateComputingTimeLabelPosition(self):
        """Update the position of computing time label at top-left corner"""
        try:
            showPref = (
                ConfigManager().getPrefsValue(
                    "PREFS", "NodeUIPreferences/ShowComputingTime"
                )
                == "true"
            )
        except Exception:
            showPref = True
        if showPref and self.computingTimeLabel.toPlainText():
            # Position at top-left corner with small margin (above the header)
            self.computingTimeLabel.setPos(3, -12)
            self.computingTimeLabel.setVisible(True)
        else:
            self.computingTimeLabel.setVisible(False)
    ...
    def updateNodeShape(self):
        ...
        # Update computing time label position
        self.updateComputingTimeLabelPosition()
    ...
    def postCreate(self, jsonTemplate=None):
        ...
        # If computing time already measured previously, show it immediately
        if self._rawNode._computingTime is not None:
            try:
                if self._rawNode._computingTime.total_seconds() < 1:
                    ms = self._rawNode._computingTime.total_seconds() * 1000
                ...
                self.computingTimeLabel.setPlainText(str(self._rawNode._computingTime))
            self.updateComputingTimeLabelPosition()

        # Ensure label state reflects current preference
        self.updateComputingTimeLabelPosition()

...
from uflow.UI.Canvas.SelectionRect import SelectionRect
...
from uflow.UI.Canvas.UINodeBase import NodeActionButtonBase
from uflow.UI.Canvas.UIPinBase import UIPinBase, PinGroup
...
from uflow.Core.PinBase import PinBase
from uflow.Core.NodeBase import NodeBase
from uflow.Input import InputManager, InputAction, InputActionType
from uflow.UI.Views.VariablesWidget import VARIABLE_TAG, VARIABLE_DATA_TAG
...
class BlueprintCanvas(CanvasBase):
    ...
    def __init__(self, graphManager, uflowInstance=None):
        ...
        self.uflowInstance.newFileExecuted.connect(self.onNewFile)
                ...
                if (
                    ...
                    if closestPin is not None:
                        if closestPin.direction == PinDirection.Input:
                            self.pressed_item.destinationPositionOverride = (
                                lambda: self.mapToScene(self.mousePos)
                            )
                        elif closestPin.direction == PinDirection.Output:
                            self.pressed_item.sourcePositionOverride = (
                                lambda: self.mapToScene(self.mousePos)
                            )
            ...
            if modifiers == QtCore.Qt.ControlModifier:
                ...
                for wire in self.connections.values():
                    if wire in self._mouseDownConnectionsSelection:
                        if (
                            wire.isSelected()
                            and QtWidgets.QGraphicsWidget.collidesWithItem(
                            ...
                            not wire.isSelected()
                            and not QtWidgets.QGraphicsWidget.collidesWithItem(
                                self._selectionRect, wire
                            )
                        ):
    ...
    def addNode(self, uiNode, jsonTemplate, parentGraph=None):
        ...
        :type uiNode: :class:`~uflow.UI.Canvas.UINodeBase.UINodeBase`
        """
...
class BlueprintCanvasWidget(QWidget):
    ...
    def __init__(self, graphManager, uflowInstance, parent=None):
        ...
        nameValidator = QtGui.QRegularExpressionValidator(
            rxLettersAndNumbers, self.leCompoundName
        )
        ...
        categoryValidator = QtGui.QRegularExpressionValidator(
            rxLetters, self.leCompoundCategory
        )
        ...
        self.uflowInstance.fileBeenLoaded.connect(self.onFileBeenLoaded)
